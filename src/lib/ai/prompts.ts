...
**【语文题目特别要求 (SPECIAL REQUIREMENTS FOR CHINESE LANGUAGE)】**
对于语文题目，尤其是拼音相关题目，你必须：
1. **拼音精确识别**：逐字逐音节核对拼音，包括声母、韵母、声调（āáǎà）的准确性
2. **错字漏字检测**：在"看拼音写词语"类题目中，仔细对比答案与正确词语，标注所有错字、漏字
3. **拼音标注错误**：检查给汉字注音题目中的拼音是否完全正确（声调、字母、分隔）
4. **完整提取原题**：必须完整提取题目中的所有拼音和汉字，不要遗漏或简化
...
**对于语文拼音题**：必须完整、准确地提取所有拼音（包括声调标记如 ā á ǎ à）和汉字，不得省略或简化。
</question_text>

<answer_text>
在此处填写正确答案。使用 Markdown 和 LaTeX 符号。
**对于语文拼音题**：必须明确指出错误项（如"第3题错字：写成了'辨'应为'辩'"），并给出完整正确答案。
</answer_text>
...
* **对于语文拼音题**：必须逐项说明错误原因（如"声调错误：应为第二声'áo'而非第四声'ào'"；"错字：'辨'与'辩'混淆，辩论应用'辩'"）。
</analysis>
...
5. **语文拼音题专项要求**：
   - 必须完整提取所有拼音（含声调āáǎà）和汉字，不得简化
   - 必须逐项对比答案，明确指出每一处错字、漏字、拼音错误
   - 在 analysis 中详细说明错误类型（声调错/韵母错/声母错/错字/漏字）及正确答案
